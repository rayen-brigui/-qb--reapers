        ...
        while true do
            if frameworkObject ~= nil then
                if frameworkObject.GetPlayerData() then
                    ...
                    frameworkObject.TriggerServerCallback("gtstore-hudv1:GetMoney", function(cash)
                        SendNUIMessage({
                            type = "update_money",
                            money = cash
                        })
...
AddEventHandler("es:addedMoney", function(a, b, m)
    ...
    TriggerEvent('gtstore-hudv1:OnAddedMoney', m - lastCash, false, "cash")
    lastCash = m
end)
...
AddEventHandler("es:removedMoney", function(a, b, m)
        ...
        TriggerEvent('gtstore-hudv1:OnAddedMoney', (m - lastCash) * -1, true, "cash")
        lastCash = m
end)
...
AddEventHandler("QBCore:Player:SetPlayerData", function(data)
    ...
    if lastCash ~= nil and lastCash ~= data.money.cash then
        if  data.money.cash > lastCash then
            TriggerEvent('gtstore-hudv1:OnAddedMoney', data.money.cash - lastCash, false , "cash")
        else
            TriggerEvent('gtstore-hudv1:OnAddedMoney', (data.money.cash - lastCash) * -1, true , "cash")
        end
        lastCash = data.money.cash
    end
    if lastBank ~= nil  and lastBank ~= data.money.bank then
        if data.money.bank > lastBank then
            TriggerEvent('gtstore-hudv1:OnAddedMoney', data.money.bank - lastBank, false, "bank")
        else
            TriggerEvent('gtstore-hudv1:OnAddedMoney', (data.money.bank - lastBank)* -1, true, "bank")
        end
    ...
    if account.name == "bank" then
        ...
        if lastBank ~= nil  and lastBank ~= money then
            if money > lastBank then
                TriggerEvent('gtstore-hudv1:OnAddedMoney',money - lastBank, false, "bank")
            else
                TriggerEvent('gtstore-hudv1:OnAddedMoney', (money - lastBank) * -1, true, "bank")
            end
            lastBank = money
        end
    end
    if account.name == 'money' then
        ...
        if lastCash ~= nil  and lastCash ~= money then
            if money > lastCash then
                ...
                TriggerEvent('gtstore-hudv1:OnAddedMoney',money - lastCash, false, "cash")
            else
                TriggerEvent('gtstore-hudv1:OnAddedMoney',  (money - lastCash ) * -1, true, "cash")
            end
            lastCash = money
        end
    end
