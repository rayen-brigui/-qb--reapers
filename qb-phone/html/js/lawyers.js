SetupLawyers = function(data) {
    ...
    if (data.length > 0) {
        ...
        if (police.length > 0) {
            $.each(police, function(i, lawyer7) {
                var element = '<div class="lawyer-list" id="lawyerid-' + i + '"> <div class="lawyer-list-firstletter" style="background-color: #0d1218c0;">' + (lawyer7.name).charAt(0).toUpperCase() + '</div> <div class="lawyer-list-fullname">' + lawyer7.name + '</div> <div class="lawyer-list-call"><i class="fas fa-phone"></i></div> </div>'
                $(".lawyers-list").append(element);
                $("#lawyerid-" + i).data('LawyerData', lawyer7);
            });
        } else {
        ...
        $(".lawyers-list").append('<br><h1 style="font-size:1.641025641025641vh; padding:1.0256410256410255vh; color:#fff; margin-top:0; width:100%; display:block; background-color: rgb(155, 15, 120);">Cardealer (' + cardealer.length + ')</h1>');

        var element = '<div class="lawyer-list"><div class="no-lawyers">There are no cardealer available.</div></div>'
        $(".lawyers-list").append(element);
